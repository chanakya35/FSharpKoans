...
module ``about strings`` =
 ...
 (* A single character is denoted using single quotes, example: 'c',
    ...
    let ApplyWhatYouLearned() =
        ...
        let getFunFacts x =
            let printMessage x doubledAndTripled = 
                sprintf "%d doubled is %d, and %d tripled is %d!" x (fst doubledAndTripled) x (snd doubledAndTripled)  
            let doubleAndTriple x = (x * 2, x * 3)
            printMessage x (doubleAndTriple x)

        let funFactsAboutThree = getFunFacts 3
        let funFactsAboutSix = getFunFacts 6

        printf "%s" (getFunFacts 42) // just for fun
